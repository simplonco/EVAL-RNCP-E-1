...
elseif ($_GET['box']=='unpublish') {
   
   $id=intval($_GET['id']);
   $connect=mysqli_connect('localhost','root','afpc1234','flip');
   $query1="UPDATE threads SET status=0 WHERE id=$id";
   $sql=mysqli_query($connect,$query1);
   header("location:topics.php");
}
...
?>
         ...
         <?php echo
         "<td><a href='topics.php?box=publish&id={$row->id}'>publier</a></td>"?>
         </tr>
         <?
         } 
        elseif ($row->status==1) {
...
?>
         ...
         <td><input  type="submit" name="delete" value="Supprimer"/></td>
         <?php echo
         "<td><a href='topics.php?box=unpublish&id={$row->id}'>non publier</a></td>"?>
         </tr>
         <?
         }
        }
